...
$(document).on('click', '#resend-user-otp', function(){
    $.get( $(this).data('href') , function( data ) {

      if(data[0] == 1) {
        toastr.success('OTP send successfully');
        $('.alert-success').show();
        $('.alert-success p').html('OTP send successfully');
        }
      else {

        toastr.error('Something Went Wrong!!');
        $('.alert-danger').show();
        $('.alert-danger ul').html('Something Went Wrong!!');
        }
...
$(document).on('submit','#totalsoldformdata',function(e){
  e.preventDefault();
if(admin_loader == 1)
  {
  $('.submit-loader').show();
}
  // $('button.totalSoldSubmit-btn').prop('disabled',true);
  disablekey();
      $.ajax({
       method:"POST",
       url:$(this).prop('action'),
        ...
        if(admin_loader == 1)
          {
            ...
            $('#totalsoldformdata input , #totalsoldformdata select , #totalsoldformdata textarea').eq(1).focus();
          }
          else
          {
            $('#show_total_order').removeClass('d-none');
            $('.alert-success').show();
            $('.alert-success p').html(data.msg);
            $('#total_sales').html(data.total_sales);
            $('#total_sold').html(data.total_sold);
        if(admin_loader == 1)
          {
            $('.submit-loader').hide();
          }
            $('button.totalSoldSubmit-btn').prop('disabled',false);
...
$(document).on('submit','#userCreateOtpForm',function(e){
  e.preventDefault();
  if(admin_loader == 1)
  {
    $('.gocover').show();
...
//   disablekey();
      $.ajax({
       ...
       success:function(data)
       {
        ...
        // if(admin_loader == 1)
        if (data == 1) {
            // window.location = mainurl + '/sales/users';
            // location.reload();
            document.getElementById("geniusform").reset();
             $('.modal').modal('hide');
            $('.alert-success').show();
            $('.alert-success p').html('New Customer Added Successfully');
            $('.gocover').hide();
          ...
          if ((data.errors)) {
            ...
            $("#userotp .modal-content .modal-body .alert-danger").focus();
            $('button.addProductSubmit-btn').prop('disabled',false);
            $('#userCreateOtpForm input , #userCreateOtpForm select , #userCreateOtpForm textarea').eq(1).focus();
            {
              $('.gocover').hide();
            }
...
$(document).on('submit','#salescartform',function(e){
  e.preventDefault();
  if(admin_loader == 1)
    {
  $('.gocover').show();
  ...
  $('button.addProductSubmit-btn').prop('disabled',true);
      $.ajax({
       ...
       success:function(data)
       {
          console.log(data);
          if ((data.errors)) {
            salescartform.parent().find('.alert-success').hide();
            salescartform.parent().find('.alert-danger').show();
            salescartform.parent().find('.alert-danger ul').html('');
            for(var error in data.errors)
            {
              $('.alert-danger ul').append('<li>'+ data.errors[error] +'</li>')
            }
            salescartform.find('input , select , textarea').eq(1).focus();
          }
          else
          {
            salescartform.parent().find('.alert-danger').hide();
            salescartform.parent().find('.alert-success').show();
            // salescartform.parent().find('.alert-success p').html(data);
