...
//**************************** CUSTOM JS SECTION ****************************************
  ...
  if (check_url_main == 'checkout') {
    ...
    $(document).on('click', '.notlogin', function(){
      toastr.error('Please Login To Buy!!');
  });
    // MODAL LOGIN FORM ENDS
    $(document).on('click', '#resend-login-otp', function(){
    ...
    $(".mregisterform").on('submit', function (e) {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data == 1) {
            window.location = mainurl + '/user/profile';
          } else {
    ...
    $("#customteaform").on('submit', function (e) {
      var $this = $(this).parent();
      e.preventDefault();
      $this.find('button.submit-btn').prop('disabled', true);
      $this.find('.alert-info').show();
...
// Wishlist Section
    ...
    // function wishlist(){
    //   toastr.error('Login to add on wishlist');
    // }
...
// Review Section
    ...
    $(document).on('submit','#customteareviewform',function(e){
      var $this = $(this);
      e.preventDefault();
      $('.gocover').show();
      $('button.submit-btn').prop('disabled',true);
          $.ajax({
           ...
           success:function(data)
           {
              ...
              $('.alert-danger ul').html('');
                ...
                $('#customteareviewform textarea').eq(0).focus();
              ...
              else
              {
                ...
                $('.alert-success p').html(data[0]);
                $('#customteareviewform textarea').eq(0).focus();
                $('#customteareviewform textarea').val('');
                setTimeout(function(){
                  location.reload();
              }, 1500);
              }
