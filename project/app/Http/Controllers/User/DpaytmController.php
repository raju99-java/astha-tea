...
class DpaytmController extends Controller
{
    public function store(Request $request)
    {
     ...
     $deposit->save();
		...
		$gateway_data = PaymentGateway::where('keyword','paytm')->first();
        $gateway = $gateway_data->convertAutoData();
        Session::put('item_number',$user->id); 
	    $data_for_request = $this->handlePaytmRequest( $item_number, $request->amount);
	...
	public function handlePaytmRequest( $order_id, $amount ) {
		...
		$paramList["MID"] = $gateway['merchant'];
		$paramList["ORDER_ID"] = $order_id;
		$paramList["CUST_ID"] = $order_id;
		$paramList["INDUSTRY_TYPE_ID"] = $gateway['industry'];
		$paramList["CHANNEL_ID"] = 'WEB';
		$paramList["TXN_AMOUNT"] = $amount;
		$paramList["WEBSITE"] = $gateway['website'];
		$paramList["CALLBACK_URL"] = route('deposit.paytm.notify');
		$paytm_merchant_key = $gateway['secret'];
    ...
    if ($gateway['sandbox_check'] == 1) {
      ...
      define('PAYTM_ENVIRONMENT', 'PROD'); // PROD
		...
		define('PAYTM_MERCHANT_KEY', $gateway['secret']); //Change this constant's value with Merchant key downloaded from portal
		define('PAYTM_MERCHANT_MID', $gateway['merchant']); //Change this constant's value with MID (Merchant ID) received from Paytm
		define('PAYTM_MERCHANT_WEBSITE', $gateway['website']); //Change this constant's value with Website name received from Paytm
