...
class GeneralSettingController extends Controller
{
    ...
    public function generalupdate(Request $request)
    {
        ...
        $data = Generalsetting::findOrFail(1);
            ...
            {
                $name = time().str_replace(' ', '', $file->getClientOriginalName());
                $data->upload($name,$file,$data->login_background);
                $input['login_background'] = $name;
            }

            if ($file = $request->file('newsletter_banner'))
            {
                $name = time().str_replace(' ', '', $file->getClientOriginalName());
                $data->upload($name,$file,$data->newsletter_banner);
                $input['newsletter_banner'] = $name;
            }
    ...
    public function newsletter_banner()
    {
        return view('admin.generalsetting.newsletter_banner');
    }

    public function login_background()
    {
        return view('admin.generalsetting.login_background');
    }
    ...
    {
       $data = Generalsetting::findOrFail(1);
       return view('admin.product.manage_type',compact('data'));
    }


    public function product_type_change_status($field,$status)
    {
        $data = Generalsetting::findOrFail(1);
        $data[$field] = $status;
        $data->update();
