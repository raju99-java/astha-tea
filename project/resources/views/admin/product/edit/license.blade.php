						...
						<div class="product-description">
							<div class="body-area">
								...
								<div class="gocover"
									...
									@if($data->user_id > 0)
												{{-- Vedndor Cross selling products --}}
												
												<div class="row">
													<div class="col-lg-12">
														<div class="left-area">
															<h4 class="heading">{{ __('Cross Selling Products') }}*</h4>
															<p class="subheading">(Added by Vendor)</p>
														</div>
													</div>
													<div class="col-lg-12">
														<select id="vcrossselling" class="js-example-basic-multiple" name="vcs_products[]" multiple="multiple">
															@foreach ($vproducts as $vproduct)
															<option value="{{$vproduct->id}}" >{{$vproduct->name}}</option>
														@endforeach
														</select>
													</div>
												</div>
											...
											<div class="row">
												<div class="col-lg-12">
													<div class="left-area">
														<h4 class="heading">{{ __('Cross Selling Products') }}*</h4>
														<p class="subheading">(Added by Admin)</p>
													</div>
												</div>
												<div class="col-lg-12">
													<select id="acrossselling" class="js-example-basic-multiple" name="acs_products[]" multiple="multiple">
...
@php
$vcsprods = $data->cross_selling_products()->where('type', 'vendor')->get();
$vselectedCsprods = [];
foreach ($vcsprods as $key => $vcsprod) {
	$vselectedCsprods[] = $vcsprod->cross_selling_product_id;
}
$json = json_encode($vselectedCsprods);
// dd($json);
@endphp
<script>
var vselectedProds = {!! $json !!};
console.log(vselectedProds);
$('#vcrossselling').val(vselectedProds).trigger('change');
</script>

@php
$acsprods = $data->cross_selling_products()->where('type', 'admin')->get();
$aselectedCsprods = [];
foreach ($acsprods as $key => $acsprod) {
	$aselectedCsprods[] = $acsprod->cross_selling_product_id;
}
$json = json_encode($aselectedCsprods);
// dd($json);
@endphp
<script>
var aselectedProds = {!! $json !!};
console.log(aselectedProds);
$('#acrossselling').val(aselectedProds).trigger('change');
</script>  
